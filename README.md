...
特定の Action を信頼済みとして age チェックを免除したい場合は `allow:` に追記してください（記載したものはゲートを無条件で通過します）。待ちたい場合は、1週間後に再度 PR を更新すれば通過します。
    ...
    steps:
      ...
      - uses: s4na/action-age-check@<SHA>  # v0
        with:
          ...
          allow: |
            actions/checkout
            s4na/action-age-check   # この Action 自身は信頼済み扱い（新しいうちの自己違反を防ぐ）
```
...
| `allow` | （空） | 信頼済みとして age チェックを免除する `owner/repo` または `owner/repo@ref`（改行区切り）。記載されたものはゲートを無条件で通過 |
